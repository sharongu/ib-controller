...
if [[ -n $got_fix_credentials && -n $got_api_credentials ]]; then
	xvfb-daemon-run "$java_path/java" -cp "$ibc_classpath" $java_vm_options $entry_point "$ibc_ini" "$fix_user_id" "$fix_password" "$ib_user_id" "$ib_password" ${mode}
elif  [[ -n $got_fix_credentials ]]; then
	xvfb-daemon-run "$java_path/java" -cp "$ibc_classpath" $java_vm_options $entry_point "$ibc_ini" "$fix_user_id" "$fix_password" ${mode}
elif [[ -n $got_api_credentials ]]; then
	xvfb-daemon-run "$java_path/java" -cp "$ibc_classpath" $java_vm_options $entry_point "$ibc_ini" "$ib_user_id" "$ib_password" ${mode}
else
	xvfb-daemon-run "$java_path/java" -cp "$ibc_classpath" $java_vm_options $entry_point "$ibc_ini" ${mode}
fi
